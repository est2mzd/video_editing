...
## replace_background 調査方針（Instruction 分析段階）

### 目的
- `replace_background` 12件について、まずは **instruction の解釈品質** を1件ずつ確認する
- この段階では **動画処理は実行しない**（処理時間が長く、切り分けが不明瞭になるため）

### 対象
- 入力データ: `/workspace/data/annotations_gt_task_ver10.json`
- 対象 action: `replace_background`（12件）
- 対象 target 代表: `background_behind_speaker`, `black_background`, `white_background`, `background`, `background_behind_man`

### 確認観点（1件ずつ）
1. action 判定が `replace_background` になるか
2. target が背景系ラベルとして扱われるか（冗長語があっても背景意図を保持できるか）
3. instruction から foreground 推定に必要な手掛かり（person / presenter / man / subject など）が拾えるか
4. params(`new_scene`, `lighting`, `depth`, `objects`) が欠損なく取得されるか
5. 背景置換関数に渡す最小実行パラメータに落とし込めるか

### 出力物（分析のみ）
- 行ごとの分析レコード（row, instruction要約, 期待action, 推定foreground手掛かり, 注意点）
- 集計サマリ（OK件数 / 要改善件数 / 代表的な曖昧表現）

### 判定基準
- OK: 背景置換としてそのまま実行可能（追加ルール不要）
- 要改善: 前景手掛かりが曖昧、または target が冗長でルール追加が必要

### 次ステップ（この方針の後）
- instruction 分析で要改善に分類された行に対して、`replace_background` 専用の正規化ルールを最小追加
- その後にのみ、動画処理テスト（12件または優先3件）へ進む

### 運用ルール
- 以降の `replace_background` 関連ログは本ファイル `/workspace/docs/submit_baseline_ver10.md` に追記する
- 追加の説明用 `.md` は作成しない

## 試行 18（replace_background: effect params の再取得・再検証）

### 目的
- `effect_name` / `effect_params` を **再取得** し、実際にどの値で置換処理が走るかを 12件で確認する
- 既存の instruction 判定結果に加えて、params の供給経路（明示指定 or default）を明確化する

### 実施内容
- 入力: `/workspace/data/annotations_gt_task_ver10.json` + `/workspace/data/annotations.jsonl`
- 参照ロジック: `dispatcher_v2.py` の `_run_replace_background()` と同じ解決規則
    1. `effect_name = params.get("effect_name", "hsv_shift")`
    2. `effect_params = params.get("effect_params", default_effect_params)`
    3. `effect_params` が dict でなければ default にフォールバック
- 判定項目:
    1. params に `effect_name` 明示があるか
    2. params に `effect_params` 明示があるか
    3. 実行時の有効値（effective）が何になるか
    4. `new_scene` 必須キー（`type/style/lighting/depth/objects`）が揃っているか

### 結果（1件ずつ）

| row | video_path | gt_target | effect_name 明示 | effect_params 明示 | 実行時 effect_name | 実行時 effect_params | 値の供給元 | new_scene keys |
|-----|------------|-----------|------------------|--------------------|--------------------|----------------------|------------|----------------|
| 3 | DaUJkmBvTKM_2_0to150.mp4 | background_behind_speaker | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 15 | cERS8z56GsU_18_46to365.mp4 | black_background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 16 | 4HYLeL_r55Q_7_0to255_scene02.mp4 | background_behind_presenter | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 18 | FbJTe47ndCU_10_0to371_scene03.mp4 | white_background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 23 | rhXDtQCpp-0_40_0to130.mp4 | background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 24 | pyJBzKCvJrk_7_0to318.mp4 | background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 39 | XXHUgddofPo_4_0to252.mp4 | background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 49 | NbWQNOzDiAI_28_0to172.mp4 | white_background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 51 | 91hePSbvai0_14_0to184.mp4 | background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 55 | FbJTe47ndCU_10_0to371.mp4 | white_background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 71 | DVJgFZqkRMI_6_0to304.mp4 | background | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |
| 79 | Bm5Pc8nJ5Sc_11_0to135.mp4 | background_behind_man | False | False | hsv_shift | hue_shift=8, saturation_scale=1.15, value_scale=0.95 | default | True |

### サマリー
- 対象件数: 12
- params で `effect_name` 明示: 0/12
- params で `effect_params` 明示: 0/12
- default 経路での実行: 12/12
- `new_scene` 必須キー充足: 12/12

### 結論
- `replace_background` は 12件すべてで、`effect_name/effect_params` を伴って実行可能
- ただし現行 GT では effect 系 params は未明示のため、実行時は全件 default（`hsv_shift` + 既定パラメータ）になる
- つまり「何もしていない」のではなく、**default の色変換設定で背景処理が実行される** 状態である

## 試行 19（replace_background: 「どう置換するか」の取得有無確認）

### 目的
- `replace_background.py` が instruction から「どのように置換するか（effect_name/effect_params）」を取得しているかを確認する

### 確認結果
- **結論: 現状は取得していない**

根拠:
1. `replace_background.py` には instruction を受け取る parser 関数がない
     - `ReplaceBackgroundConfig` は `effect_name`, `effect_params`, `foreground_prompt` を入力として受け取る設計
2. `dispatcher_v2.py` の `_run_replace_background()` では、instruction を使わず params/default から effect を決めている
     - `effect_name = params.get("effect_name", "hsv_shift")`
     - `effect_params = params.get("effect_params", default_effect_params)`
3. foreground 推定は instruction ではなく YOLO 画像推論 (`infer_foreground_prompt_by_yolo(frames)`) に依存

### 影響
- instruction に「どんな背景にするか」「どんな色味にするか」が書かれていても、
    現状の replace_background 実行には直接反映されない（task params 側に定義されている場合のみ反映）

### この試行での実行有無
- instruction 解析のみ実施（動画処理は未実施）

### 次対応（方針）
- 必要なら `replace_background.py` か `dispatcher_v2.py` に
    instruction -> (`effect_name`, `effect_params`) を抽出する軽量ルールを追加する

## 試行 20（補足: replace 先の色変数は実際に適用されている）

### 目的
- 「replace 先の色の変数などは設定されているので、何かやっているはず」という点をコード経路で再確認する

### 確認結果
- **結論: 実際に処理している（適用されている）**

根拠:
1. `dispatcher_v2.py` の `_run_replace_background()` で `params` から `effect_name` / `effect_params` を取得している
    - `effect_name = params.get("effect_name", "hsv_shift")`
    - `effect_params = params.get("effect_params", default_effect_params)`
2. 上記を `ReplaceBackgroundConfig` に入れて `run_replace_background_color_pipeline(cfg)` に渡している
3. `replace_background.py` の `apply_background_effect()` 内で
    - `EFFECT_REGISTRY[effect_name](frame_bgr, effect_params)` を実行し、
    - 背景マスク領域にのみ合成している

### 解釈
- したがって、**replace の「やり方（色味・色変換パラメータ）」は既に効いている**
- ただし供給元は instruction 直接解析ではなく、現状は `params`（または default）である

### この試行での実行有無
- コード経路の再確認のみ（動画処理は未実施）

## 試行 21（replace_background: instruction から色/スタイルを effect_name に反映）

### 目的
- `replace_background.py` に、instruction から背景の色・スタイル表現を抽出し、`effect_name` / `effect_params` として保存・利用できる機能を追加する

### 実装
- 更新: `/workspace/src/postprocess/replace_background.py`
    - 新規関数 `infer_background_effect_from_instruction(instruction)` を追加
    - ルールベースで instruction を解析し、`(effect_name, effect_params)` を返す
        1. blur/defocus/bokeh 系 -> `gaussian_blur`
        2. 色キーワード（green/pink/blue/red/orange/purple） -> `color_tint`
        3. 明るさ/コントラスト語 -> `brightness_contrast`
        4. スタイル語（neon/cyberpunk/warm/cinematic/pastel/drama など） -> preset 解決
- 更新: `/workspace/src/postprocess/dispatcher_v2.py`
    - `_run_replace_background()` に `instruction` と `logger` を渡すよう変更
    - effect 解決優先順位を明示化:
        1. params 明示 (`effect_name` / `effect_params`)
        2. instruction 抽出結果
        3. default（`hsv_shift`）
    - 解決結果を params に保存:
        - `_resolved_effect_name`
        - `_resolved_effect_params`
        - `_resolved_effect_source` (`params` / `instruction` / `default`)

### 動作確認
1. 構文確認
    - `PYTHONPATH=/workspace:/workspace/src python -m py_compile /workspace/src/postprocess/replace_background.py /workspace/src/postprocess/dispatcher_v2.py`
2. 機能確認（軽量モック）
    - instruction 解析関数と dispatcher 解決順を確認
    - 結果:
        - pink style -> `source=instruction`, `effect_name=color_tint`
        - blur background -> `source=instruction`, `effect_name=gaussian_blur`
        - params 明示あり -> `source=params`（instruction より優先）
        - 該当語なし -> `source=default`, `effect_name=hsv_shift`
3. 回帰確認（既存テスト）
    - `PYTHONPATH=/workspace:/workspace/src /usr/bin/python /workspace/src/test/test_dispatcher_v2_single_video.py --row-index 0 --max-frames 8`
    - 既存 dispatcher 経路が継続実行できることを確認

### 結論
- `replace_background` は instruction から背景色/スタイルを effect に変換して扱えるようになった
- 同時に、既存 params 指定がある場合はそれを優先するため、従来の互換性を維持している

### 12件の effect_name 結果表（replace_background 全件）

判定条件:
- inferred: `infer_background_effect_from_instruction(instruction)` の返り値
- resolved: dispatcher 優先順位（params > instruction > default）で最終採用された値

| row | video_path | effect_name明示 | effect_params明示 | inferred_effect_name | inferred_effect_params | resolved_effect_name | resolved_effect_params | source |
|-----|------------|-----------------|-------------------|----------------------|------------------------|----------------------|------------------------|--------|
| 3 | DaUJkmBvTKM_2_0to150.mp4 | False | False | hsv_shift | {"hue_shift": -6, "saturation_scale": 0.82, "value_scale": 1.05} | hsv_shift | {"hue_shift": -6, "saturation_scale": 0.82, "value_scale": 1.05} | instruction |
| 15 | cERS8z56GsU_18_46to365.mp4 | False | False | brightness_contrast | {"contrast": 1.08, "brightness": 8} | brightness_contrast | {"contrast": 1.08, "brightness": 8} | instruction |
| 16 | 4HYLeL_r55Q_7_0to255_scene02.mp4 | False | False | - | - | hsv_shift | {"hue_shift": 8, "saturation_scale": 1.15, "value_scale": 0.95} | default |
| 18 | FbJTe47ndCU_10_0to371_scene03.mp4 | False | False | color_tint | {"tint_bgr": [60, 120, 200], "strength": 0.22} | color_tint | {"tint_bgr": [60, 120, 200], "strength": 0.22} | instruction |
| 23 | rhXDtQCpp-0_40_0to130.mp4 | False | False | color_tint | {"tint_bgr": [220, 120, 40], "strength": 0.22} | color_tint | {"tint_bgr": [220, 120, 40], "strength": 0.22} | instruction |
| 24 | pyJBzKCvJrk_7_0to318.mp4 | False | False | gaussian_blur | {"ksize": 17, "sigma": 0.0} | gaussian_blur | {"ksize": 17, "sigma": 0.0} | instruction |
| 39 | XXHUgddofPo_4_0to252.mp4 | False | False | color_tint | {"tint_bgr": [220, 120, 40], "strength": 0.15} | color_tint | {"tint_bgr": [220, 120, 40], "strength": 0.15} | instruction |
| 49 | NbWQNOzDiAI_28_0to172.mp4 | False | False | color_tint | {"tint_bgr": [220, 120, 40], "strength": 0.22} | color_tint | {"tint_bgr": [220, 120, 40], "strength": 0.22} | instruction |
| 51 | 91hePSbvai0_14_0to184.mp4 | False | False | - | - | hsv_shift | {"hue_shift": 8, "saturation_scale": 1.15, "value_scale": 0.95} | default |
| 55 | FbJTe47ndCU_10_0to371.mp4 | False | False | gaussian_blur | {"ksize": 17, "sigma": 0.0} | gaussian_blur | {"ksize": 17, "sigma": 0.0} | instruction |
| 71 | DVJgFZqkRMI_6_0to304.mp4 | False | False | gaussian_blur | {"ksize": 17, "sigma": 0.0} | gaussian_blur | {"ksize": 17, "sigma": 0.0} | instruction |
| 79 | Bm5Pc8nJ5Sc_11_0to135.mp4 | False | False | gaussian_blur | {"ksize": 17, "sigma": 0.0} | gaussian_blur | {"ksize": 17, "sigma": 0.0} | instruction |

集計:
- source=params: 0/12
- source=instruction: 10/12
- source=default: 2/12

## 試行 22（default色変化の強化 + replace_background 全12件を1/3フレーム実行）

### 目的
- default 経路の見た目変化が分かりやすいよう、`replace_background` の default effect を強める
- `replace_background` 対象 12件を全件実行し、各動画で **最低 1/3 フレーム** を処理する

### 実装
- default effect の変更（より視認しやすい色変化）
    - `dispatcher_v2.py` の default を以下へ変更
        - `effect_name: color_tint`
        - `effect_params: {"tint_bgr": (30, 200, 255), "strength": 0.35}`
    - `replace_background.py` の `ReplaceBackgroundConfig` 既定値も同じ値へ統一
- 1/3フレーム実行の標準化
    - `test_replace_background_gt_cases.py` の既定動作を「各動画の総フレーム数の 1/3（最小1）」へ変更
    - `--max-frames` の意味を整理
        - `-1`（default）: 1/3
        - `0`: 全フレーム
        - `>0`: 指定フレーム数
...
- 出力:
    - `/workspace/logs/test/replace_background_gt_cases/20260410_113201/`
    - `report.json`, `run.log`, 出力動画 12本

### 結果
- 成功件数: **12/12**
- 1/3フレーム処理（used/total）:

| row | used/total | output_video |
|-----|------------|--------------|
| 3 | 41/125 | row003_replace_background_DaUJkmBvTKM_2_0to150.mp4 |
| 15 | 50/150 | row015_replace_background_cERS8z56GsU_18_46to365.mp4 |
| 16 | 40/120 | row016_replace_background_4HYLeL_r55Q_7_0to255_scene02.mp4 |
| 18 | 40/120 | row018_replace_background_FbJTe47ndCU_10_0to371_scene03.mp4 |
| 23 | 41/125 | row023_replace_background_rhXDtQCpp-0_40_0to130.mp4 |
| 24 | 50/150 | row024_replace_background_pyJBzKCvJrk_7_0to318.mp4 |
| 39 | 50/150 | row039_replace_background_XXHUgddofPo_4_0to252.mp4 |
| 49 | 41/125 | row049_replace_background_NbWQNOzDiAI_28_0to172.mp4 |
| 51 | 41/125 | row051_replace_background_91hePSbvai0_14_0to184.mp4 |
| 55 | 40/120 | row055_replace_background_FbJTe47ndCU_10_0to371.mp4 |
| 71 | 41/125 | row071_replace_background_DVJgFZqkRMI_6_0to304.mp4 |
| 79 | 40/120 | row079_replace_background_Bm5Pc8nJ5Sc_11_0to135.mp4 |

- 実行時 effect 解決（run.log 抜粋）:

| row | source | effect_name | effect_params |
|-----|--------|-------------|---------------|
| 3 | instruction | hsv_shift | {'hue_shift': -6, 'saturation_scale': 0.82, 'value_scale': 1.05} |
| 15 | instruction | brightness_contrast | {'contrast': 1.08, 'brightness': 8} |
| 16 | default | color_tint | {'tint_bgr': (30, 200, 255), 'strength': 0.35} |
| 18 | instruction | color_tint | {'tint_bgr': (60, 120, 200), 'strength': 0.22} |
| 23 | instruction | color_tint | {'tint_bgr': (220, 120, 40), 'strength': 0.22} |
| 24 | instruction | gaussian_blur | {'ksize': 17, 'sigma': 0.0} |
| 39 | instruction | color_tint | {'tint_bgr': (220, 120, 40), 'strength': 0.15} |
| 49 | instruction | color_tint | {'tint_bgr': (220, 120, 40), 'strength': 0.22} |
| 51 | default | color_tint | {'tint_bgr': (30, 200, 255), 'strength': 0.35} |
| 55 | instruction | gaussian_blur | {'ksize': 17, 'sigma': 0.0} |
| 71 | instruction | gaussian_blur | {'ksize': 17, 'sigma': 0.0} |
| 79 | instruction | gaussian_blur | {'ksize': 17, 'sigma': 0.0} |

### 分析
- 1/3フレーム要件は全件で満たした（120f 動画は40f、125f は41f、150f は50f）
- instruction で effect が推定できる行が多数（10/12）で、default へ落ちるのは 2件
- default を `color_tint` 強め設定へ変更したことで、default 経路でも背景の色変化が視認しやすくなった
- 既存優先順位（params > instruction > default）は維持されているため、明示 params を使う既存ケースへの互換性は保たれる

## 試行 23（`target=background` と foreground mask の関係確認）

### 目的
- `run.log` に `target=background` が出ているとき、前景 mask が `target` を使って作られているかを確認する。

### 結論
- 現在の `replace_background` 実装では、前景 mask の生成に `target` は使っていない。
- 前景 mask は YOLO 推定した `foreground_prompt` を使って GroundingDINO+SAM で作成している。

### 根拠（コード）
- `dispatcher_v2.py`:
    - `target` は正規化して `params["target"]` に保存している。
    - ただし `replace_background` 実行時は `infer_foreground_prompt_by_yolo(frames)` の結果を `foreground_prompt` として `ReplaceBackgroundConfig` に渡している。
- `replace_background.py`:
    - `estimate_background_mask_gdino_sam(..., foreground_prompt=...)` で検出。
    - `detect_all_boxes(..., text_prompt=foreground_prompt)` を使って前景 box を検出し、SAM mask の反転で背景 mask を作る。

### run.log の解釈
- `target=background` はアノテーション由来の target 表示であり、それ自体は異常ではない。
- この値がそのまま前景検出 prompt に使われる設計ではないため、`target=background` だから全失敗、とは直結しない。
- 実行結果としては当該バッチは `Done. 12/12 succeeded`。

### 補足
- 見た目上の失敗（人物側まで変わる/背景が十分変わらない）がある場合、まず疑うべきは `target` 文字列より、YOLO による foreground 推定のズレ。

## 試行 24（背景領域が大きすぎる問題の再確認 + 12件再出力）

### 目的
- 「背景領域が大きすぎる」という観点で、`replace_background` の挙動を再確認する。
- `replace_background` 12件をすべて **1/3 フレーム** で再出力する。
- 上記の結果と、以降の検証目的を `.md` に記録する。

### 実行
- コマンド:
    - `./scripts/run_test_replace_background_gt.sh`
- 出力ディレクトリ:
    - `/workspace/logs/test/replace_background_gt_cases/20260410_130810/`
- 完了ログ:
    - `Done. 12/12 succeeded. report=/workspace/logs/test/replace_background_gt_cases/20260410_130810/report.json`

### 結果
- 12/12 成功、全件で 1/3 フレーム処理（40/120, 41/125, 50/150）を満たした。
...
- 背景領域比率の追加集計（先頭8フレーム平均、`mask_ratio_summary.json`）:

| row | avg_bg_ratio_first8 | avg_fg_ratio_first8 | 備考 |
|-----|----------------------|---------------------|------|
| 3 | 0.7517 | 0.2483 | - |
| 15 | 0.7846 | 0.2154 | - |
| 16 | 0.7829 | 0.2171 | - |
| 18 | 0.6604 | 0.3396 | - |
| 23 | 0.8828 | 0.1172 | 背景比率高め |
| 24 | 0.5716 | 0.4284 | - |
| 39 | 0.6699 | 0.3301 | - |
| 49 | 1.0000 | 0.0000 | foreground 未検出に近い |
| 51 | 0.6992 | 0.3008 | - |
| 55 | 0.6601 | 0.3399 | - |
| 71 | 0.6965 | 0.3035 | - |
| 79 | 0.7096 | 0.2904 | - |
...
関連ファイル:
- `report.json`: `/workspace/logs/test/replace_background_gt_cases/20260410_130810/report.json`
- `run.log`: `/workspace/logs/test/replace_background_gt_cases/20260410_130810/run.log`
- `mask_ratio_summary.json`: `/workspace/logs/test/replace_background_gt_cases/20260410_130810/mask_ratio_summary.json`

### 分析
- 12件の再実行と 1/3 フレーム要件は達成。
- 一部ケースで背景領域が大きい傾向が確認された（特に row 49 は avg_bg_ratio=1.0）。
- `target=background` そのものではなく、foreground 推定（YOLO→GDINO/SAM）側の取りこぼしが主因と考えられる。

### 以降の目的
- foreground 未検出時のフォールバックを強化する（`prev_fg_mask` の再利用条件見直し、人物領域下限の導入）。
- `foreground_prompt` を固定 `person .` だけでなく、instruction/target の語彙を補助入力にして検出安定性を比較する。
- row 49 を再現ケースとして、mask 面積しきい値と box 選択規則（サイズ・IoU）のチューニングを優先する。

### 訂正: `DEBUG_MODE=True` は白背景化ではない
- 前回の記述だけでは、「`DEBUG_MODE=True` にすると背景を真っ白にする」と読める余地があったが、**実際のコードはそうなっていない**。
- 現在の `replace_background.py` での `DEBUG_MODE=True` の役割は、**デバッグ用の画像・JSONを保存することだけ** である。

#### 該当コード（`/workspace/src/postprocess/replace_background.py`）
- `22-25` 行付近:
    - `DEBUG_MODE = True`
    - `DEBUG_MAX_FRAMES = 8`
    - コメントも「per-frame debug artifacts を保存」となっている。
- `413-416` 行付近:
    - `debug_enabled = bool(DEBUG_MODE)`
    - `debug_dir = ..._debug`
    - `debug_enabled` のときだけ debug directory を作る。
- `434-452` 行付近:
    - `frame_xxxx_orig.jpg`
    - `frame_xxxx_bg_mask.png`
    - `frame_xxxx_overlay.jpg`
    - `frame_xxxx_edited.jpg`
    - および `bg_ratio`, `fg_ratio`, `fg_detected` を `debug_metrics` に記録している。
- `466-481` 行付近:
    - `debug_report.json` を保存し、summary に `debug_mode`, `debug_dir`, `debug_report` を追加して返している。

#### 背景を白くする処理はどこか
- 背景を白くする処理は `DEBUG_MODE` ではなく、**effect 設定** で決まる。
- 実際に背景へ effect を適用している箇所は `427-431` 行付近で、ここでは
    - `effect_name=cfg.effect_name`
    - `effect_params=cfg.effect_params`
  を `apply_background_effect()` に渡している。
- したがって、背景を真っ白にするには `cfg.effect_name='color_tint'` かつ `cfg.effect_params={'tint_bgr': (255, 255, 255), 'strength': 1.0}` のような **明示指定** が必要。

#### 今回の白背景デバッグ実行について
- 白背景化は repo の常設コードではなく、動作確認用にこちらで一時的に実行した direct run の `ReplaceBackgroundConfig` で指定した。
- つまり、**現状の repo コードには「DEBUG_MODE=True なら背景を白にする」という分岐は存在しない**。

#### 解釈
- `DEBUG_MODE=True`:
    - debug artifact 保存 ON
    - 背景色そのものは変えない
- 白背景化:
    - `effect_name/effect_params` の設定でのみ決まる
    - `DEBUG_MODE` とは独立

## 試行 25（DEBUG_MODE の巻き戻し + `DEBUG_MODE_` 色指定 + 12件実行）

### 目的
- これまでの DEBUG_MODE の使い方（debug artifact 保存）をやめて、シンプルなデバッグ方式に戻す。
- `DEBUG_MODE_=True` のときだけ背景色を指定できるようにする。
- その状態で `replace_background` 12件を 1/3 フレーム実行し、mp4 出力を確認する。

### コード修正
更新ファイル:
- `/workspace/src/postprocess/replace_background.py`

変更点:
1. 旧 DEBUG_MODE の撤去
- debug 画像保存（`*_orig.jpg`, `*_bg_mask.png`, `*_overlay.jpg`, `*_edited.jpg`）
- `debug_report.json` 生成
- summary への `debug_dir`, `debug_report` 付与

2. 新しいシンプルデバッグ
- グローバル変数を追加
    - `DEBUG_MODE_ = True`
    - `DEBUG_BG_COLOR_BGR = (255, 255, 255)`
- `run_replace_background_color_pipeline(cfg)` 内で
    - 通常時: `cfg.effect_name/cfg.effect_params` を使用
    - `DEBUG_MODE_ == True` 時のみ強制上書き:
        - `effect_name = 'color_tint'`
        - `effect_params = {'tint_bgr': DEBUG_BG_COLOR_BGR, 'strength': 1.0}`

3. summary の挙動
- `summary['debug_mode'] = bool(DEBUG_MODE_)`
- `DEBUG_MODE_ == True` 時は `summary['debug_color_bgr']` を追加

### 実行
- コマンド:
    - `./scripts/run_test_replace_background_gt.sh`
- 出力ディレクトリ:
    - `/workspace/logs/test/replace_background_gt_cases/20260410_134410/`
- 完了ログ:
    - `Done. 12/12 succeeded. report=/workspace/logs/test/replace_background_gt_cases/20260410_134410/report.json`

### 結果
- 成功件数: **12/12**
- 1/3 フレーム処理は全件達成（40/120, 41/125, 50/150）

出力 mp4（12本）:
- `row003_replace_background_DaUJkmBvTKM_2_0to150.mp4`
- `row015_replace_background_cERS8z56GsU_18_46to365.mp4`
- `row016_replace_background_4HYLeL_r55Q_7_0to255_scene02.mp4`
- `row018_replace_background_FbJTe47ndCU_10_0to371_scene03.mp4`
- `row023_replace_background_rhXDtQCpp-0_40_0to130.mp4`
- `row024_replace_background_pyJBzKCvJrk_7_0to318.mp4`
- `row039_replace_background_XXHUgddofPo_4_0to252.mp4`
- `row049_replace_background_NbWQNOzDiAI_28_0to172.mp4`
- `row051_replace_background_91hePSbvai0_14_0to184.mp4`
- `row055_replace_background_FbJTe47ndCU_10_0to371.mp4`
- `row071_replace_background_DVJgFZqkRMI_6_0to304.mp4`
- `row079_replace_background_Bm5Pc8nJ5Sc_11_0to135.mp4`

関連ファイル:
- `/workspace/logs/test/replace_background_gt_cases/20260410_134410/report.json`
- `/workspace/logs/test/replace_background_gt_cases/20260410_134410/run.log`

### 補足
- `DEBUG_MODE_` の目的は「背景色の強制上書き」に限定し、挙動を単純化した。
- これにより、デバッグ時に色確認をしやすくしつつ、通常時ロジック（instruction/params/default の解決）と切り分けできる。

## 試行 26（背景変化が弱く見える要因と強度設定箇所の特定）

### 目的
- 「mask は作れているのに背景変化が弱く見える」原因が、合成ロジック由来か `strength` 設定由来かをコード上で切り分ける。
- どの値を調整すれば見え方が強くなるかを、実際の設定箇所とあわせて明確化する。

### 調査結果
1. `color_tint` の強度は `strength` で直接制御される
- ファイル: `/workspace/src/postprocess/replace_background.py`
- 該当: `effect_color_tint()`
    - `strength = float(params.get('strength', 0.25))`
    - `cv2.addWeighted(frame_bgr, 1.0 - strength, tint, strength, 0.0)`
- 意味: `strength` が小さいほど元フレーム寄り（変化が弱く見える）。

2. 背景だけへの適用はマスク合成で決まる
- ファイル: `/workspace/src/postprocess/replace_background.py`
- 該当: `apply_background_effect()`
    - `out = frame*(1-mask) + effected*mask`
- 意味: マスク外（前景）は変化しない。マスク内だけ effect が見える。

3. dispatcher 側デフォルトは `strength=0.35`
- ファイル: `/workspace/src/postprocess/dispatcher_v2.py`
- 該当: `_run_replace_background()`
    - `default_effect_params = {"tint_bgr": (30, 200, 255), "strength": 0.35}`
- 意味: デフォルトは中程度のブレンド。映像によっては「変化したが弱い」に見える。

4. instruction 推定でも強度は 0.15 / 0.22 / 0.35 に収まる
- ファイル: `/workspace/src/postprocess/replace_background.py`
- 該当: `_extract_color_tint_params_from_instruction()`
    - `slight/subtle/light` 系: `0.15`
    - 通常: `0.22`
    - `strong/deep/vivid` 系: `0.35`
- 意味: 明示指定しない場合、強度が低〜中に収まりやすい。

5. `DEBUG_MODE_` では `strength=1.0` を強制
- ファイル: `/workspace/src/postprocess/replace_background.py`
- 該当: `run_replace_background_color_pipeline()`
    - `DEBUG_MODE_ == True` 時は `effect_name='color_tint'`
    - `effect_params={'tint_bgr': DEBUG_BG_COLOR_BGR, 'strength': 1.0}`
- 意味: デバッグ時に背景変化が見えない場合は、強度不足より mask 側（領域/検出）の問題を優先疑い。

### 結論
- 背景変化の見え方は主に `strength` で決まり、現在の通常経路では `0.15〜0.35` が中心のため弱く見えるケースがあり得る。
- 強く見せるには、`dispatcher_v2.py` の default `strength` か、各行 `effect_params` の `strength` を上げるのが最短。
- `DEBUG_MODE_` で `strength=1.0` でも変化が乏しい場合は、blend 強度ではなく foreground 推定（YOLO→GDINO/SAM）のマスク品質を優先して調査する。

## 試行 27（strength 未反映ケースのコード確認 + default strength=0.9 再実行）

### 目的
- 「strength が効いていないところがある」事象をコード上で確認する。
- `default strength` を `0.9` に変更し、replace_background 12件（1/3フレーム）を再実行する。

### コードチェック結果（未反映に見える理由）
1. `strength` は **`effect_name=color_tint` のときだけ** 直接使われる
- ファイル: `/workspace/src/postprocess/replace_background.py`
- 該当: `effect_color_tint()`
    - `cv2.addWeighted(..., strength, ...)` で反映

2. `effect_name` が `gaussian_blur` / `brightness_contrast` / `hsv_shift` の場合、`color_tint` の `strength` は使われない
- 該当:
    - `effect_gaussian_blur()` は `ksize/sigma`
    - `effect_brightness_contrast()` は `contrast/brightness`
    - `effect_hsv_shift()` は `hue_shift/saturation_scale/value_scale`
- つまり、`strength` を上げても、これらの effect には影響しない。

3. dispatcher では `params > instruction > default` の優先順位
- ファイル: `/workspace/src/postprocess/dispatcher_v2.py`
- 該当: `_run_replace_background()`
    - instruction から effect 推定できたケースは default を使わない
    - そのため default `strength` を変更しても、`source=instruction` 行は見え方が変わらない場合がある

### 変更
- ファイル: `/workspace/src/postprocess/dispatcher_v2.py`
- 変更内容:
    - `default_effect_params["strength"]: 0.55 -> 0.9`

### 実行
- コマンド:
    - `./scripts/run_test_replace_background_gt.sh`
- 出力ディレクトリ:
    - `/workspace/logs/test/replace_background_gt_cases/20260410_145132/`
- 完了ログ:
    - `Done. 12/12 succeeded. report=/workspace/logs/test/replace_background_gt_cases/20260410_145132/report.json`

### 結果
- 成功件数: **12/12**
- default strength=0.9 が適用された行（ログ確認）:
    - row16: `source=default, effect_name=color_tint, strength=0.9`
    - row51: `source=default, effect_name=color_tint, strength=0.9`
- `source=instruction` の行は、instruction 由来 effect が優先されるため default 0.9 の影響は限定的。

### 補足（Notebook 更新）
- `/workspace/notebook/replace_background_02_test.ipynb` の比較セルを更新:
    - before は `/workspace/data/videos/*` を参照
    - after は今回の実行結果 `/workspace/logs/test/replace_background_gt_cases/20260410_145132/*` を参照
    - 各セルで `instruction/action/target/色変化方法(予定 effect)` を print するようにした

## 試行 28（強度指定の現状整理 + グローバル強度変数の導入）

### 目的
- 上記（試行27）内容を本 .md に追記したうえで、色をブレンドする際の強度指定方法の現状を明文化する。
- `replace_background.py` 冒頭の Global 変数で、各手法の色味/効果強度を一括調整できるように実装する。

### 1) 現状の強度指定方法（導入前の整理）
1. dispatcher の default 値
- ファイル: `/workspace/src/postprocess/dispatcher_v2.py`
- `default_effect_params` で `color_tint.strength` を指定（現在 0.9）

2. instruction 推定値
- ファイル: `/workspace/src/postprocess/replace_background.py`
- `infer_background_effect_from_instruction()` が effect を選択
- `color_tint` のときのみ `strength` を持つ（例: 0.15 / 0.22 / 0.35）

3. effect ごとの実パラメータ
- `color_tint`: `strength`
- `hsv_shift`: `hue_shift`, `saturation_scale`, `value_scale`
- `brightness_contrast`: `contrast`, `brightness`
- `gaussian_blur`: `ksize`, `sigma`
- そのため、`strength` 変更は `color_tint` にしか効かない。

### 2) 実装した Global 変数（ファイル冒頭）
更新ファイル:
- `/workspace/src/postprocess/replace_background.py`

追加した変数:
- `GLOBAL_COLOR_TINT_INTENSITY = 1.0`
- `GLOBAL_HSV_SHIFT_INTENSITY = 1.0`
- `GLOBAL_BRIGHTNESS_CONTRAST_INTENSITY = 1.0`
- `GLOBAL_GAUSSIAN_BLUR_INTENSITY = 1.0`
- `GLOBAL_CLAHE_INTENSITY = 1.0`

意味:
- `1.0`: 既存挙動そのまま
- `>1.0`: 効果を強める
- `0.0`: その手法の効果をほぼ無効化（neutral 寄り）

### 3) コード反映内容
1. 新規関数 `_apply_global_effect_intensity(effect_name, params)` を追加
- 手法ごとにパラメータをスケール:
    - `color_tint`: `strength *= GLOBAL_COLOR_TINT_INTENSITY`（0..1 clip）
    - `hsv_shift`: 中立値 1.0 を基準にスケール
    - `brightness_contrast`: `contrast` は 1.0 基準、`brightness` は線形スケール
    - `gaussian_blur`: `ksize`, `sigma` をスケール
    - `clahe`: `clip_limit` をスケール

2. `run_replace_background_color_pipeline()` で 1回適用
- `effect_name/effect_params` 解決後に、上記関数を通して実効パラメータを決定
- 以降の全フレーム処理はこの実効パラメータを使用

### 4) 使い方
1. `replace_background.py` 冒頭の Global 変数を変更
- 例:
    - `GLOBAL_COLOR_TINT_INTENSITY = 1.2`（色かぶせを強く）
    - `GLOBAL_HSV_SHIFT_INTENSITY = 0.7`（HSV変化を弱く）
    - `GLOBAL_BRIGHTNESS_CONTRAST_INTENSITY = 1.3`（明暗変化を強く）

2. 通常どおり実行
- `./scripts/run_test_replace_background_gt.sh`

3. 見方
- `source=instruction` の行でも、手法ごとの Global 変数は適用される
- ただし `effect_name` 自体の選択（params/instruction/default 優先順位）は従来どおり

### 5) 備考
- 構文確認済み:
    - `python -m py_compile /workspace/src/postprocess/replace_background.py`

## 試行 29（Global 強度設定で 12 ケース再実行）

### 出力フォルダ（先頭明記）
- `/workspace/logs/test/replace_background_gt_cases/20260410_152549/`

### 目的
- Global 強度変数を適用した現行設定で、replace_background 12 ケースを 1/3 フレーム実行して mp4 を再生成する。
- 強め設定時でも全ケース完走することを確認する。

### 変化点
- 対象ファイル: `/workspace/src/postprocess/replace_background.py`
- 今回の Global 設定値:
    - `GLOBAL_COLOR_TINT_INTENSITY = 1.0`
...
- コマンド:
    - `./scripts/run_test_replace_background_gt.sh`
- 完了ログ:
    - `Done. 12/12 succeeded. report=/workspace/logs/test/replace_background_gt_cases/20260410_152549/report.json`
...
### 追記メモ
- ログ上の default color_tint ケースでは `strength=0.5` が確認された（row16, row51）。
- これは `dispatcher default(0.9)` に `GLOBAL_COLOR_TINT_INTENSITY` が乗算され、実効値として反映された結果。
## 試行 30（row71 で変化が弱い件のQ&A整理）

### 質問
- Notebook 表示では row71 が
    - `color_method_source=default`
    - `color_method_effect=color_tint`
    - `strength=0.55`
  なのに、見た目の変化が弱いのはなぜか。

### 回答（整理）
1. Notebook の表示値と実処理がずれている可能性
- 当該セルの表示は `instruction` が空文字扱いになっており、`source=default` 判定になっていた。
- 一方、実行ログ側では row71 が `source=instruction` で解決されるケースがある。
- その場合は `default color_tint` ではなく、instruction 推定 effect が使われるため、表示想定と結果が一致しない。

2. after_path が古い実行フォルダを参照していた
- Notebook 例では `after_path=/workspace/logs/test/replace_background_gt_cases/20260410_140359/...` を参照していた。
- これは最新設定の出力ではないため、期待した変化強度とズレる。

3. row71 は blur 系になりやすく、見た目差分が小さい場合がある
- 実処理で `gaussian_blur` が選ばれると、背景がすでにぼけ気味の素材では差分が目視しづらい。
- この場合、`color_tint` の strength を見ても実際の変化説明にはならない。

### DEBUG_MODE と通常モードの違い（再整理）
- `DEBUG_MODE_=True`:
    - `effect_name='color_tint'`, `tint_bgr=DEBUG_BG_COLOR_BGR`, `strength=1.0` を強制。
    - 「白背景化に成功した」時はこの強制経路で動いている。
- `DEBUG_MODE_=False`（通常）:
    - `params > instruction > default` で effect を解決。
    - その後 Global 強度倍率を適用。
    - したがって、常に白背景になるわけではない。

### 今後の運用メモ
- Notebook 側の `instruction/action/target/effect_source` 表示は、実行ログの resolved 値と揃えること。
- 比較時は必ず最新出力フォルダ（今回なら `20260410_152549`）を参照すること。
