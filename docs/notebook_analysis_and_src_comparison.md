...
以下コマンドで full video を 10フレーム間引きで実行し、4ケースすべて成功した。

```bash
python3 src/test/gt_unit_test.py \
    --run-default-scenarios \
    --frame-stride 10 \
    --output-dir logs/test/notebook_match_v2 \
    --retry 1
```

### 成功したシナリオ
- add_object_05_last_add_object: OK
- apply_style_03_last_apply_style: OK
- dolly_in_02_dolly_in: OK
- zoom_in_01_zoom_in: OK

### 生成された出力
- `logs/test/notebook_match_v2/1s9DER1bpm0_10_0to213__add_object.mp4`
- `logs/test/notebook_match_v2/94msufYZzaQ_26_0to273__apply_style.mp4`
- `logs/test/notebook_match_v2/wyzi9GNZFMU_0_0to121__dolly_in.mp4`
- `logs/test/notebook_match_v2/_pQAUwy0yWs_0_119to277__zoom_in.mp4`

### 補足
- 実行時に HuggingFace/PyTorch の warning は出るが、4ケースとも処理完了して出力 mp4 生成まで確認済み。
