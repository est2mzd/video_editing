...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        default="/workspace/configs/submit_baseline_ver03.yaml",
    )
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise TypeError(f"Config must be dict: {path}")
    return data
...
def ellipse_mask(
    ...
    cv2.ellipse(mask, center_xy, radius_xy, 0, 0, 360, value, -1)
    return mask


def detect_primary_face(frame: np.ndarray) -> tuple[int, int, int, int] | None:
    ...
    def score(face: np.ndarray) -> float:
        ...
        fx = x + fw / 2.0
        fy = y + fh / 2.0
        dist = abs(fx - cx) / max(w, 1) + abs(fy - cy) / max(h, 1)
        return area - dist * area * 0.7
...
def estimate_subject_mask(
    ...
    if face_box is not None:
        x, y, fw, fh = face_box
        body_cx = x + fw // 2
        body_cy = min(h - 1, y + int(fh * cfg.get("body_center_y_scale", 1.8)))
        body_rx = min(w // 2 - 1, int(fw * cfg.get("body_radius_x_scale", 1.4)))
        body_ry = min(h // 2 - 1, int(fh * cfg.get("body_radius_y_scale", 2.6)))
        body_mask = ellipse_mask(h, w, (body_cx, body_cy), (max(10, body_rx), max(10, body_ry)))
        rect_x0 = max(0, x - int(fw * cfg.get("grabcut_pad_x", 1.2)))
        rect_y0 = max(0, y - int(fh * cfg.get("grabcut_pad_top", 0.8)))
        rect_x1 = min(w - 1, x + fw + int(fw * cfg.get("grabcut_pad_x", 1.2)))
        rect_y1 = min(h - 1, y + int(fh * cfg.get("grabcut_pad_bottom", 3.6)))
    else:
        body_mask = ellipse_mask(h, w, (w // 2, int(h * 0.58)), (int(w * 0.24), int(h * 0.34)))
        rect_x0 = int(w * 0.2)
        rect_y0 = int(h * 0.08)
        rect_x1 = int(w * 0.8)
        rect_y1 = int(h * 0.96)

    rect = (rect_x0, rect_y0, max(2, rect_x1 - rect_x0), max(2, rect_y1 - rect_y0))
    gc_mask = np.zeros((h, w), np.uint8)
    bgd_model = np.zeros((1, 65), np.float64)
    fgd_model = np.zeros((1, 65), np.float64)
    try:
        cv2.grabCut(frame, gc_mask, rect, bgd_model, fgd_model, int(cfg.get("grabcut_iter", 2)), cv2.GC_INIT_WITH_RECT)
        fg = np.where((gc_mask == cv2.GC_FGD) | (gc_mask == cv2.GC_PR_FGD), 255, 0).astype(np.uint8)
    except Exception:
        fg = np.zeros((h, w), dtype=np.uint8)

    combined = np.maximum(fg, body_mask)
    combined = np.maximum(combined, (center * float(cfg.get("center_subject_prior", 0.42))).astype(np.uint8))
    return cv2.GaussianBlur(combined, (0, 0), cfg.get("subject_blur_sigma", 11))
...
def infer_anchor_xy(
    ...
    moments = cv2.moments(subject_mask)
    if moments["m00"] > 0:
...
def blend_anchor_xy(
    ...
    cx = int(round((w // 2) * (1.0 - mix) + anchor_xy[0] * mix))
    cy = int(round((h // 2) * (1.0 - mix) + anchor_xy[1] * mix))
    return int(np.clip(cx, 0, w - 1)), int(np.clip(cy, 0, h - 1))


def build_video_context(frame: np.ndarray, cfg: dict[str, Any]) -> VideoContext:
    face_box = detect_primary_face(frame)
    subject_mask = estimate_subject_mask(frame, face_box, cfg)
    anchor_xy = infer_anchor_xy(frame, face_box, subject_mask)
    return VideoContext(face_box=face_box, subject_mask=subject_mask, anchor_xy=anchor_xy, config=cfg)
...
def apply_zoom(
    ...
    if anchor_xy is None:
        anchor_xy = (w // 2, h // 2)
    ax = int(np.clip(anchor_xy[0], 0, w - 1))
    ay = int(np.clip(anchor_xy[1], 0, h - 1))
    src = frame
    if scale >= 1.0:
        crop_w = max(2, int(round(w / scale)))
        crop_h = max(2, int(round(h / scale)))
        x0 = int(np.clip(ax - crop_w // 2, 0, max(0, w - crop_w)))
        y0 = int(np.clip(ay - crop_h // 2, 0, max(0, h - crop_h)))
        ...
        x0 = int(np.clip(ax - scaled_w // 2, 0, max(0, w - scaled_w)))
        y0 = int(np.clip(ay - scaled_h // 2, 0, max(0, h - scaled_h)))
...
def apply_perspective_tilt(
    ...
    if anchor_xy is None:
        anchor_xy = (w // 2, h // 2)
    margin = max(6, int(round(h * strength * (0.35 + 0.65 * progress))))
    anchor_shift = int((anchor_xy[0] - w // 2) * 0.18)
    src = np.float32([[0, 0], [w - 1, 0], [0, h - 1], [w - 1, h - 1]])
    if low_angle:
        dst = np.float32([
            [margin + anchor_shift, 0],
            [w - 1 - margin + anchor_shift, 0],
            [0, h - 1],
            [w - 1, h - 1],
        ])
    else:
        dst = np.float32([
            [0, 0],
            [w - 1, 0],
            [margin + anchor_shift, h - 1],
            [w - 1 - margin + anchor_shift, h - 1],
        ])
...
def mask_from_face_region(
    ...
    h, w = frame.shape[:2]
    if face_box is None:
        return build_center_weight(h, w)
    x, y, fw, fh = face_box
    mask = np.zeros((h, w), dtype=np.uint8)
    if region == "hair":
        center = (x + fw // 2, max(0, y + fh // 8))
        axes = (max(8, int(fw * 0.72)), max(8, int(fh * 0.72)))
        cv2.ellipse(mask, center, axes, 0, 180, 360, 255, -1)
    elif region == "hat":
        center = (x + fw // 2, max(0, y + fh // 8))
        axes = (max(8, int(fw * 0.85)), max(8, int(fh * 0.52)))
        cv2.ellipse(mask, center, axes, 0, 180, 360, 255, -1)
    elif region == "tie":
        pts = np.array([
            [x + int(fw * 0.44), y + int(fh * 1.02)],
            [x + int(fw * 0.56), y + int(fh * 1.02)],
            [x + int(fw * 0.66), y + int(fh * 2.05)],
            [x + int(fw * 0.50), y + int(fh * 2.55)],
            [x + int(fw * 0.34), y + int(fh * 2.05)],
        ], dtype=np.int32)
        cv2.fillConvexPoly(mask, pts, 255)
    elif region == "face":
        cv2.ellipse(mask, (x + fw // 2, y + fh // 2), (max(8, fw // 2), max(8, fh // 2)), 0, 0, 360, 255, -1)
    return cv2.GaussianBlur(mask, (0, 0), 7)


def apply_color_retarget_with_mask(frame: np.ndarray, target_color: str | None, alpha_mask: np.ndarray) -> np.ndarray:
    if not target_color:
        return frame
    ...
    if target_color in {"violet", "purple"}:
        overlay_hsv[..., 1] = np.clip(np.maximum(overlay_hsv[..., 1], 150), 0, 255)
        overlay_hsv[..., 2] = np.clip(overlay_hsv[..., 2] * 1.14 + 10, 0, 255)
    elif target_color == "black":
        overlay_hsv[..., 1] = np.clip(overlay_hsv[..., 1] * 0.28, 0, 255)
        overlay_hsv[..., 2] = np.clip(overlay_hsv[..., 2] * 0.32, 0, 255)
    elif target_color in {"white", "silver"}:
        overlay_hsv[..., 1] = np.clip(overlay_hsv[..., 1] * 0.18, 0, 255)
        overlay_hsv[..., 2] = np.clip(overlay_hsv[..., 2] * 1.22 + 12, 0, 255)
    else:
        overlay_hsv[..., 1] = np.clip(np.maximum(overlay_hsv[..., 1], 120), 0, 255)
        overlay_hsv[..., 2] = np.clip(overlay_hsv[..., 2] * 1.08 + 8, 0, 255)
    overlay = cv2.cvtColor(overlay_hsv, cv2.COLOR_HSV2BGR)
    return alpha_blend(frame, overlay, alpha_mask)
...
def filter_background_change(frame: np.ndarray, subject_mask: np.ndarray, cfg: dict[str, Any]) -> np.ndarray:
    h, w = frame.shape[:2]
    blurred = cv2.GaussianBlur(frame, (0, 0), 15)
    overlay = np.zeros_like(frame)
    overlay[:] = tuple(cfg.get("bg_base_bgr", [60, 75, 90]))
    for y in range(h):
        ratio = y / max(h - 1, 1)
        overlay[y, :, 0] = cfg.get("bg_grad_b0", 90) + int(cfg.get("bg_grad_b_scale", 20) * ratio)
        overlay[y, :, 1] = cfg.get("bg_grad_g0", 90) + int(cfg.get("bg_grad_g_scale", 35) * ratio)
        overlay[y, :, 2] = cfg.get("bg_grad_r0", 105) + int(cfg.get("bg_grad_r_scale", 50) * ratio)
    bg = cv2.addWeighted(blurred, float(cfg.get("bg_blur_mix", 0.42)), overlay, float(cfg.get("bg_overlay_mix", 0.58)), 0)
    fg_alpha = cv2.GaussianBlur(subject_mask, (0, 0), cfg.get("fg_blur_sigma", 17))
...
def apply_quantity_increase_ver03(frame: np.ndarray, context: VideoContext) -> np.ndarray:
    h, w = frame.shape[:2]
    ax, ay = context.anchor_xy
    src_rect = (
        int(np.clip(ax - w * 0.10, 0, w - 2)),
        int(np.clip(ay - h * 0.06, 0, h - 2)),
        int(np.clip(ax + w * 0.10, 1, w - 1)),
        int(np.clip(ay + h * 0.10, 1, h - 1)),
    )
    out = frame.copy()
    placements = [
        ((int(w * 0.16), int(h * 0.74)), 0.58),
        ((int(w * 0.80), int(h * 0.69)), 0.54),
        ((int(w * 0.52), int(h * 0.82)), 0.46),
        ((int(w * 0.34), int(h * 0.77)), 0.42),
    ]
    for center, scale in placements:
        out = duplicate_patch(out, src_rect, center, scale)
    return out
...
def smooth_fill_region(frame: np.ndarray, mask: np.ndarray, blur_sigma: float = 9.0) -> np.ndarray:
    blurred = cv2.GaussianBlur(frame, (0, 0), blur_sigma)
    return alpha_blend(frame, blurred, mask)


def remove_region_with_background_hint(frame: np.ndarray, mask: np.ndarray, white_bias: bool) -> np.ndarray:
    out = smooth_fill_region(frame, mask, blur_sigma=11.0)
    if white_bias:
        white = np.full_like(frame, 242)
        return alpha_blend(out, white, np.clip(mask.astype(np.float32) * 0.72, 0, 255).astype(np.uint8))
    return out


def apply_instance_removal_ver03(frame: np.ndarray, record: AnnotationRecord, context: VideoContext) -> np.ndarray:
    h, w = frame.shape[:2]
    instruction = record.instruction.lower()
    if "background" in instruction or "white surface" in instruction:
        if context.face_box is not None:
            x, y, fw, fh = context.face_box
            cx = min(w - 1, x + int(fw * 1.4))
            cy = min(h - 1, y + int(fh * 1.6))
        else:
            cx, cy = int(w * 0.72), int(h * 0.48)
        mask = ellipse_mask(h, w, (cx, cy), (max(10, int(w * 0.10)), max(10, int(h * 0.12))))
        mask = feather_mask(mask, 9)
        return remove_region_with_background_hint(frame, mask, white_bias="white" in instruction)
    mask = ellipse_mask(h, w, (int(w * 0.78), int(h * 0.54)), (max(10, int(w * 0.09)), max(10, int(h * 0.14))))
    mask = feather_mask(mask, 9)
    return remove_region_with_background_hint(frame, mask, white_bias=False)


def apply_human_motion_ver03(frame: np.ndarray, record: AnnotationRecord, context: VideoContext, progress: float) -> np.ndarray:
    h, w = frame.shape[:2]
    if context.face_box is None:
        return frame
    x, y, fw, fh = context.face_box
    instruction = record.instruction.lower()
    out = frame.copy()
    if "smile" in instruction or "facial expression" in instruction:
        roi = out[y:y + fh, x:x + fw].copy()
        if roi.size == 0:
            return out
        mouth_mask = np.zeros((fh, fw), dtype=np.uint8)
        cv2.ellipse(mouth_mask, (fw // 2, int(fh * 0.72)), (max(6, int(fw * 0.22)), max(4, int(fh * 0.10))), 0, 0, 360, 255, -1)
        smile_overlay = roi.copy()
        smile_overlay[:, :, 2] = np.clip(smile_overlay[:, :, 2] * (1.0 + 0.05 * progress) + 8 * progress, 0, 255)
        roi = alpha_blend(roi, smile_overlay, feather_mask((mouth_mask * (0.35 + 0.45 * progress)).astype(np.uint8), 5))
        warp = np.float32([[0, 0], [fw - 1, 0], [0, fh - 1], [fw - 1, fh - 1]])
        lift = int(fh * 0.06 * progress)
        dst = np.float32([[0, 0], [fw - 1, 0], [0, fh - 1 - lift], [fw - 1, fh - 1 - lift]])
        roi2 = cv2.warpPerspective(roi, cv2.getPerspectiveTransform(warp, dst), (fw, fh), borderMode=cv2.BORDER_REFLECT101)
        out[y:y + fh, x:x + fw] = roi2
        return out
    if "wave" in instruction or "raises his left hand" in instruction:
        bx = max(0, x - int(fw * 0.8))
        by = max(0, y + int(fh * 0.9))
        bw = min(w - bx, int(fw * 0.95))
        bh = min(h - by, int(fh * 1.2))
        roi = out[by:by + bh, bx:bx + bw]
        if roi.size == 0:
            return out
        shift = int(bh * 0.12 * progress)
        mat = np.float32([[1, 0, int(bw * 0.06 * progress)], [0, 1, -shift]])
        moved = cv2.warpAffine(roi, mat, (bw, bh), borderMode=cv2.BORDER_REFLECT101)
        mask = np.zeros((bh, bw), dtype=np.uint8)
        cv2.ellipse(mask, (int(bw * 0.28), int(bh * 0.35)), (max(6, int(bw * 0.22)), max(6, int(bh * 0.32))), -20, 0, 360, 255, -1)
        out[by:by + bh, bx:bx + bw] = alpha_blend(roi, moved, feather_mask((mask * (0.40 + 0.35 * progress)).astype(np.uint8), 7))
        return out
    return out
...
def apply_localized_attribute_edit(
    frame: np.ndarray,
    ...
    instruction = record.instruction.lower()
    if "hair" in instruction:
        alpha = mask_from_face_region(frame, context.face_box, "hair")
        return apply_color_retarget_with_mask(frame, target_color, alpha)
    if "necktie" in instruction or "tie" in instruction:
        alpha = mask_from_face_region(frame, context.face_box, "tie")
        return apply_color_retarget_with_mask(frame, target_color, alpha)
    if "hat" in instruction or "beanie" in instruction:
        alpha = mask_from_face_region(frame, context.face_box, "hat")
        return apply_color_retarget_with_mask(frame, target_color, alpha)
    return apply_color_retarget_with_mask(frame, target_color, context.subject_mask)


def apply_instance_replacement_ver02(
    frame: np.ndarray,
    record: AnnotationRecord,
    context: VideoContext,
) -> np.ndarray:
    ...
    if "hat" in instruction or "beanie" in instruction:
        ...
        alpha = mask_from_face_region(frame, context.face_box, "hat")
        return apply_color_retarget_with_mask(frame, target_color, alpha)
    return apply_instance_replacement(frame, record.instruction)
...
def apply_edit(
    ...
    cfg = context.config
    zoom_anchor = blend_anchor_xy(frame, context.anchor_xy, float(cfg.get("zoom_anchor_mix", 0.32)))
    angle_anchor = blend_anchor_xy(frame, context.anchor_xy, float(cfg.get("angle_anchor_mix", 0.28)))
    ...
    if cls == "Camera Motion Editing":
        if sub in {"Zoom in", "Dolly in"}:
            edited = apply_zoom(
                ...
                progress,
                zoom_in=True,
                anchor_xy=zoom_anchor,
                max_scale=float(cfg.get("zoom_in_max_scale", 1.28)),
            )
        elif sub == "Zoom out":
            edited = apply_zoom(
                edited,
                ...
                zoom_in=False,
                anchor_xy=zoom_anchor,
                min_scale=float(cfg.get("zoom_out_min_scale", 0.82)),
            )
        elif sub == "Arc shot":
            ...
            edited = apply_zoom(
                ...
                zoom_in=True,
                anchor_xy=zoom_anchor,
                max_scale=float(cfg.get("arc_zoom_scale", 1.18)),
            )
    elif cls == "Camera Angle Editing":
        if sub == "Low angle":
            edited = apply_perspective_tilt(
                edited,
                progress,
                low_angle=True,
                anchor_xy=angle_anchor,
                strength=float(cfg.get("low_angle_strength", 0.075)),
            )
        elif sub == "High angle":
            edited = apply_perspective_tilt(
                edited,
                ...
                low_angle=False,
                anchor_xy=angle_anchor,
                strength=float(cfg.get("high_angle_strength", 0.07)),
            )
    elif cls == "Style Editing":
        edited = apply_style(edited, sub)
    elif cls == "Visual Effect Editing":
        if sub == "Background Change":
            edited = filter_background_change(edited, context.subject_mask, cfg)
        ...
        edited = apply_localized_attribute_edit(edited, record, context)
    elif cls == "Quantity Editing" and sub == "Increase":
        edited = apply_quantity_increase_ver03(edited, context)
    elif cls == "Instance Motion Editing":
        if sub == "Human motion":
            edited = apply_human_motion_ver03(edited, record, context, progress)
    elif cls == "Instance Editing":
        if sub == "Instance Replacement":
            edited = apply_instance_replacement_ver02(edited, record, context)
        elif sub == "Instance Insertion" and "microphone" in record.instruction.lower():
            edited = overlay_microphone(edited)
        elif sub == "Instance Removal":
            edited = apply_instance_removal_ver03(edited, record, context)
...
def process_video(
    ...
    while True:
        ...
        if not ok:
            break
        edited = apply_edit(frame, record, context, written, frame_count)
...
def main() -> int:
    ...
    for row_index, record in iter_records(records, args.start_index, args.limit):
        ...
        result = process_video(record, input_path, output_path, codec=codec, cfg=cfg)
