#!/usr/bin/env python3
"""LLM trial 010 parser (LLM-first, no singlefile import).
...
class InstructionParserV3LLMTrial010:
    ...
    def _heuristic_target(self, instruction: str, action: str) -> str:
        ...
        if action == "change_camera_angle":
            m = re.search(
                r"\b(man|woman|boy|girl|person|speaker|subject"
                r"|dancer|player|chef|athlete)",
                text,
            )
            if m:
                return m.group(1)
...
def build_parser() -> InstructionParserV3LLMTrial010:
    return InstructionParserV3LLMTrial010(LLMConfig())
