#!/usr/bin/env python3
"""LLM trial 009 parser (LLM-first, no singlefile import).
...
class InstructionParserV3LLMTrial009:
    ...
    def _locked_action(self, instruction: str) -> str | None:
        ...
        # replace (i.e. within ~5 words after 'replace')
        if re.search(
            r"\breplace\s+(?:\w+\s+){0,5}background\b", text
        ):
    ...
    def _heuristic_target(self, instruction: str, action: str) -> str:
        ...
        if action == "edit_motion":
            m = re.search(
                r"\b(man|woman|boy|girl|baby|person|speaker|subject)\b",
                text,
            )
            if m:
                return m.group(1)
...
def build_parser() -> InstructionParserV3LLMTrial009:
    return InstructionParserV3LLMTrial009(LLMConfig())
