#!/usr/bin/env python3
"""Mixed parser trial 003.

Fixes over trial002:
1. apply_style locked: added Studio Ghibli / aesthetic / inspired / transform-aesthetic patterns
2. _locked_action: increase.*amount → increase_amount (was add_object - bug)
3. _locked_action: modify.*color|change.*color added before edit_motion fallback
4. LLM rescue in rule_main restricted: camera actions (dolly_in/out, zoom_in/out) cannot rescue edit_motion
...
class InstructionParserV3MixedTrial003:
    ...
    def _normalize_prediction(
        ...
        _CAMERA_ACTIONS = {
            ...
            "orbit_camera", "change_camera_angle",
        }
        if self.cfg.mix_mode == "rule_main":
            action = rule_action
            # Let LLM rescue low-confidence generic edits only.
            # Do NOT allow camera actions to rescue edit_motion (causes
            # false dolly_in from e.g. "tilts her head to the right").
            if (
                action == "edit_motion"
                and llm_action in ALLOWED_ACTIONS
                and llm_action != "edit_motion"
                and llm_action not in _CAMERA_ACTIONS
                and not rule_high
            ):
                action = llm_action
            target = rule_target
    ...
    def _locked_action(self, instruction: str) -> str | None:
        text = instruction.lower()
        # apply_style: style keywords override everything
        if re.search(
            r"\bcyberpunk\b|\bneon sign\b|\bvintage\b|\banime\b"
            r"|\bwatercolor\b|\boil.?paint\b|\bsketch\b"
            r"|\bghibli\b|\baesthetic\b|\binspired\b"
            r"|\bpixel.?art\b|\bukiyo\b",
            text,
        ):
            return "apply_style"
        # transform.*aesthetic / transform.*style also → apply_style
        if re.search(
            r"\btransform\b.*\b(aesthetic|style|look|feel)\b",
            text,
        ):
            return "apply_style"
        ...
        if re.search(r"\breplace\b.*\bwith\b", text):
            return "replace_object"
        # increase_amount: "increase the amount/number/count of X" → increase_amount
        if re.search(r"\bincrease\b.*\bamount\b", text):
            return "increase_amount"
        # add_object: "increase the number/count of" or "add more"
        if re.search(
            r"\bincrease\b.*\b(number|count)\b|\badd more\b",
            text,
        ):
            return "add_object"
        if re.search(r"\bdolly.?in\b", text):
        ...
        if re.search(r"\blow angle\b|\bhigh angle\b|\bcamera angle\b", text):
            return "change_camera_angle"
        # change_color: match both "change/modify X color" patterns
        if re.search(
            r"\bchange\b.*\bcolor\b|\bmodify\b.*\bcolor\b|\brecolor\b",
            text,
        ):
...
def build_parser() -> InstructionParserV3MixedTrial003:
    return InstructionParserV3MixedTrial003(LLMConfig())
