#!/usr/bin/env python3
"""Single-file parser consolidated from v3 trials.

This file removes multi-level imports across trial files and keeps the latest
rule logic in one class.
"""
...
def build_knowledge_db_v3(gt_path: Path) -> dict[str, Any]:
    """Build static pattern DB for v3 parser family."""
    _ = gt_path  # reserved for interface compatibility
    ...
    noun_patterns = {
        ...
        "location": (
            ...
            r"forest|"
            r"city|street|indoor|outdoor)\b"
        ),
        "visual": (
    ...
    print("✓ Knowledge DB v3 構築完了（single-file）")
...
class InstructionParserV3SingleFile:
    ...
    def _extract_target(self, instruction: str, action: str) -> str:
        ...
        # generic fallback from noun patterns
...
def build_parser() -> InstructionParserV3SingleFile:
    kb = build_knowledge_db_v3(GT_PATH)
    return InstructionParserV3SingleFile(kb)
...
def main() -> None:
    ...
    samples = [
        ...
        (
            "Increase the number of exhausted animals in the scene by adding "
            "more rhinos and buffalos"
        ),
        "Transform the entire video into a vibrant Cyberpunk style",
    ]
    print(
        json.dumps(
