#!/usr/bin/env python3
"""Trial 004 based on v3.

Single countermeasure:
- Fix regex boundary typo in trial scoring logic so boosts/penalties actually
  work (use \b, not \\b pattern text).
"""
...
from parse.prototype_instruction_parser_v3_improved_trials.prototype_instruction_parser_v3_improved_trial003 import (
    InstructionParserV3Trial003,
)
...
class InstructionParserV3Trial004(InstructionParserV3Trial003):
    def _infer_action(self, instruction: str) -> str:
        ...
        has_style_intent = bool(
            re.search(r"\b(style|cyberpunk|ukiyo|pixel|oil painting)\b", text)
        )
...
def build_parser() -> InstructionParserV3Trial004:
    kb = build_knowledge_db_v3(GT_PATH)
    return InstructionParserV3Trial004(kb)
